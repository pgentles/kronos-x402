...
export function getPrice(toolName: string): number {
  return PRICING[toolName] || 0.05;
}

export function processPayment(toolName: string): PaymentResult {
...
export function getFullPricing(): { tool: string; price: number; description: string }[] {
  return [
