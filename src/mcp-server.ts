...
export interface McpContext { prepaid?: boolean; requestId?: string; }
...
export async function executeTool(name: string, args: Record<string, unknown>, _ctx?: McpContext): Promise<McpToolResult> {
  try {
    switch (name) {
    ...
    if (e.name === 'ZodError') {
      return { content: [{ type: 'text', text: JSON.stringify({ error: 'Invalid input', details: e.errors.map((err: any) => ({ path: err.path.join('.'), message: err.message })) }) }], isError: true };
    }
