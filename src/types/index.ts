...
export const SYMBOLS = ['BTC/USD', 'ETH/USD', 'SOL/USD', 'XRP/USD', 'ADA/USD', 'DOGE/USD', 'AVAX/USD', 'LINK/USD', 'DOT/USD', 'MATIC/USD'] as const;
...
export function normalizeSymbol(input: string): Symbol {
  const upper = input.toUpperCase().replace('/USD', '').replace('-USD', '').trim();
  return SYMBOL_MAP[upper] || ('BTC/USD' as Symbol);
}
