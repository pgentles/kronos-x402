...
import { normalizeSymbol } from '../types/index.js';
import { getPrice, getPriceChange24h, getVolumeChange24h, getVolatility, getRegime, getMarketState, getSignalStrength, getCooldown, getDecisionConfidence, auditDecision } from '../data/market-sim.js';
// In-memory decision store for audit trail
const DECISION_STORE = new Map();
export function checkTradePreflight(symbolInput) {
...
export function getDecision(symbolInput) {
    const symbol = normalizeSymbol(symbolInput);
    const price = getPrice(symbol);
    const change = getPriceChange24h(symbol);
    ...
    const bias = change > 1 ? 'upward' : change < -1 ? 'downward' : 'neutral';
    const decision = {
        symbol,
...
export function auditTradeDecision(decisionId, windowHours = 1) {
    ...
    const result = auditDecision(initialPrice, decision.directionalBias, windowHours);
    return {
        decisionId,
...
export function getCryptoSignals(symbolInput) {
    const symbol = normalizeSymbol(symbolInput);
    return {
        symbol,
...
export function getCryptoForecast(symbolInput) {
    const symbol = normalizeSymbol(symbolInput);
    const price = getPrice(symbol);
    const vol = getVolatility(symbol);
...
export function getCryptoRisk(symbolInput) {
    const symbol = normalizeSymbol(symbolInput);
    const vol = getVolatility(symbol);
    ...
    const factors = [];
    if (riskScore > 70)
        factors.push('Elevated volatility regime');
    if (Math.abs(change) > 5)
        factors.push('Significant 24h price movement');
    if (volumeChange < -20)
        factors.push('Declining volume');
    if (riskScore < 30)
...
export function getSignalHistory(symbolInput, hours = 24) {
    const symbol = normalizeSymbol(symbolInput);
    const now = Date.now();
    const result = [];
...
export function reviewSignalAnomaly(symbolInput, window = '24h', features) {
    const symbol = normalizeSymbol(symbolInput);
    ...
    const score = Math.round((Math.abs(defaults.price_change) * 2) +
        (Math.abs(defaults.volume_change) * 0.1) +
        (defaults.volatility * 100) +
        (defaults.risk_score * 0.3));
    let reviewLabel = 'normal_review';
    if (score > 150)
        reviewLabel = 'critical_review';
    else if (score > 100)
        reviewLabel = 'elevated_review';
    else if (score > 50)
        reviewLabel = 'review';
    const drivers = [];
    if (Math.abs(defaults.price_change) > 5)
        drivers.push('Extreme price movement');
    if (Math.abs(defaults.volume_change) > 50)
        drivers.push('Abnormal volume activity');
    if (defaults.volatility > 0.05)
        drivers.push('High volatility regime');
    if (defaults.risk_score > 60)
        drivers.push('Elevated risk environment');
    return {
        symbol,
        window,
        reviewLabel,
